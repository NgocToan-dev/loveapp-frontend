    ...
    <div class="d-flex align-center mb-6">
      ...
      <div>
        <h1 class="text-h4 font-weight-bold text-pink mb-1">
          <v-icon icon="mdi-heart-edit" class="mr-2"></v-icon>
          {{ $t('memories.editMemory') || 'Edit Memory' }}
        </h1>
        <p class="text-body-1 text-medium-emphasis">
          {{ $t('memories.editSubtitle') || 'Update your precious memory' }}
        </p>
      </div>
    </div>

    <v-form ref="form" v-model="valid" @submit.prevent="updateMemory">
      <v-card>
        ...
        <v-card-actions class="px-6 pb-6">
          ...
          >
            {{ $t('memories.update') || 'Update Memory' }}
          </v-btn>
        </v-card-actions>
      </v-card>
    </v-form>
...
const loadMemory = async () => {
  ...
  if (memory) {    // Load memory data into form
    formData.value = {
      title: memory.title,
      description: memory.description || '',
      date: typeof memory.date === 'string' ? memory.date.split('T')[0] : new Date(memory.date).toISOString().split('T')[0],
      location: typeof memory.location === 'string' ? memory.location : memory.location?.name || '',
    ...
    const fetchedMemory = memoriesStore.memories.find(m => m.id === memoryId)
    if (fetchedMemory) {      formData.value = {
        title: fetchedMemory.title,
        description: fetchedMemory.description || '',
        date: typeof fetchedMemory.date === 'string' ? fetchedMemory.date.split('T')[0] : new Date(fetchedMemory.date).toISOString().split('T')[0],
        location: typeof fetchedMemory.location === 'string' ? fetchedMemory.location : fetchedMemory.location?.name || '',
        category: fetchedMemory.category || '',
        tags: fetchedMemory.tags || [],
        isPrivate: fetchedMemory.isPrivate || false
      }
    } else {
      // Memory not found, redirect to memories list
