    ...
    <div class="blog-detail-page">
      ...
      <div v-else-if="!currentPost" class="not-found-container">
        <div class="not-found-content bg-gray-50 border border-gray-200 rounded-lg p-6 text-center">
          ...
          <h2 class="text-xl font-semibold text-gray-800 mb-2">{{ $t('blog.detail.not_found') }}</h2>
      ...
      <div v-else class="content max-w-4xl mx-auto">
        ...
        <div class="page-header flex justify-between items-start mb-8">
          <div class="header-content flex items-center space-x-4">
            ...
            <div class="header-badges flex space-x-2">
              <Badge
                :label="formatDate(currentPost.publishedAt || currentPost.createdAt)"
                variant="secondary"
              />
              <Badge
        ...
        <article class="blog-post-content bg-white rounded-lg shadow-sm border p-8">
          ...
          <header class="post-header mb-8">
            ...
            <!-- Excerpt -->
            <div v-if="currentPost.excerpt" class="excerpt text-xl text-gray-600 mb-6 leading-relaxed">
              {{ currentPost.excerpt }}
            </div>

            <!-- Metadata -->
            <div class="metadata flex flex-wrap items-center gap-6 text-sm text-gray-500 border-b border-gray-200 pb-6">
              ...
              <div v-if="currentPost.updatedAt !== currentPost.createdAt" class="flex items-center">
          ...
          <div class="post-content">
            ...
            <div v-else-if="currentPost.content" class="prose prose-lg max-w-none whitespace-pre-wrap">
              {{ currentPost.content }}
            </div>
          ...
          <!-- Tags -->
          <div v-if="currentPost.tags && currentPost.tags.length > 0" class="tags-section mt-8 pt-6 border-t border-gray-200">
          ...
          <!-- Social Share -->
          <div v-if="currentPost.status === 'published'" class="social-share mt-8 pt-6 border-t border-gray-200">
      ...
      <!-- Image Viewer Modal -->
      <Modal v-model="showImageViewer" :title="currentPost?.title || ''" size="lg" :close-on-backdrop="true">
...
const estimatedReadTime = computed(() => {
  if (!currentPost.value?.content) return 5
  const wordsPerMinute = 200
  const wordCount = currentPost.value.content.split(/\s+/).length
  return Math.ceil(wordCount / wordsPerMinute)
})

const relatedPosts = computed(() => {
  if (!currentPost.value || !posts) return []
...
const handleDelete = async () => {
  ...
  try {
    ...
    showSuccess(t('blog.delete.success'))
    router.push('/blog')
  } catch (error) {
    showError(t('blog.delete.error'))
  }
}
...
const handleEditRelated = (relatedPost: BlogPost) => {
...
const shareToTwitter = () => {
  const url = encodeURIComponent(window.location.href)
  const text = encodeURIComponent(`${currentPost.value?.title} - ${currentPost.value?.excerpt || ''}`)
