<template>
  <v-card max-width="800" rounded="xl">
    <v-card-title class="text-h5 font-weight-bold pa-6">
      <v-icon color="primary" class="mr-2">mdi-note-plus</v-icon>
      {{ $t('notes.createNew') || 'Create New Note' }}
    </v-card-title>
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Emit success with the created note
    emit('confirm', newNote)
  } catch (error) {
