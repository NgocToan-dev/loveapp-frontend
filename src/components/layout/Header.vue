      ...
      <div class="flex justify-between items-center h-16">
        ...
        <div class="flex items-center">
          <router-link to="/" class="flex items-center space-x-2">
            ...
            <span class="font-romantic text-2xl bg-gradient-to-r from-primary-600 to-spring-green bg-clip-text text-transparent">LoveApp</span>
          </router-link>
        </div>
        ...
        <div class="hidden md:flex items-center space-x-4">
          ...
          >
            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-3.5-3.5L15 9a6 6 0 10-12 0l1.5 4-3.5 3.5H6M11 20a2 2 0 01-4 0" />
            </svg>
            <span v-if="upcomingRemindersCount > 0" class="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center">
      ...
      <div v-if="uiStore.isMobileMenuOpen" class="md:hidden py-2">
        <div class="flex flex-col space-y-1">
          ...
          <div class="border-t border-gray-200 pt-4 mt-4">
            ...
            <button 
              @click="handleLogout"
              class="block w-full text-left text-red-600 hover:text-red-700 px-3 py-2 rounded-md text-base font-medium"
            >
              {{ $t('common.actions.signOut') }}
            </button>
          </div>
...
const upcomingRemindersCount = computed(() => 
  remindersStore.upcomingReminders.length
)
